...
if [ -n "${REPLICATION_SLAVE}" ]; then
    ...
    if [ -n "${MYSQL_PORT_3306_TCP_ADDR}" ] && [ -n "${MYSQL_PORT_3306_TCP_PORT}" ]; then
        if [ ! -f /replication_configured ]; then
            ...
            mysql -uroot -e "slave start"
            echo "=> Done!"
            mysqladmin -uroot shutdown

            # set master back
            mysql -uroot --host=${MYSQL_PORT_3306_TCP_ADDR} -e "slave stop"
            mysql -uroot -e "CHANGE MASTER TO MASTER_HOST='${MYSQL_MASTER_IP}',MASTER_USER='${MYSQL_ENV_REPLICATION_USER}',MASTER_PASSWORD='${MYSQL_ENV_REPLICATION_PASS}',MASTER_PORT=${MYSQL_PORT_3306_TCP_PORT}, MASTER_CONNECT_RETRY=30"
            mysql -uroot --host=${MYSQL_PORT_3306_TCP_ADDR} -e "slave start"
            mysqladmin -uroot --host=${MYSQL_PORT_3306_TCP_ADDR} shutdown
